    ...
    pub fn password_analysis(&mut self, password: &str) {
        ...
        lazy_static! {
            static ref EMAILS: Regex = Regex::new(
                ...
                */
                r"(?i)[a-z0-9+[\.]{0,1}?+]{1,32}@([a-z0-9]+([\-]{1}[a-z0-9]+)*\.(com\.[a-z]{2,3}?|com|net\.[a-z]{2,3}?|net|gov\.[a-z]{2,3}?|gov|org\.[a-z]{2,3}?|org|tn|it|fr|jp|ar|au|de|ru|br|in|uk|ar|nl|nz|es|id|io|it|ca|ch))",
                )
            .unwrap();
